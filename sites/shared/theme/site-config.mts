...
export function siteConfig(directory:URL) {
 ...
 return defineConfig({
  ...
  themeConfig:{logo:'/logo.png',siteTitle:site.name,site,contact:site.contact,nav:nav.nav,sidebar:nav.sidebar,
   ...
   search:{provider:'local',options:{miniSearch:{_splitIntoSections:courseSearchSections,options:{tokenize:cjkTokenize},searchOptions:{tokenize:cjkTokenize,combineWith:'AND',prefix:false,fuzzy:0}},translations:{button:{buttonText:'搜索',buttonAriaLabel:'搜索'},modal:{noResultsText:'没有找到相关内容',resetButtonTitle:'清除',footer:{selectText:'打开',navigateText:'切换',closeText:'关闭'}}}}},
