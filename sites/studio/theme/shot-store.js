...
export function installShots(raw) {
 ...
 const snapshot=()=>({schema:SCHEMA,version:'5.6',siteId:data.site.id,updatedAt:Date.now(),hideMissing:state.hideMissing,records:JSON.parse(JSON.stringify(state.records))})
 const normalize=s=>{
  const r=parseSnapshot(s,data.site.id,specs)
  return {schema:SCHEMA,version:'5.6',siteId:data.site.id,updatedAt:s.updatedAt||null,hideMissing:r.hideMissing,records:r.records}
 }
 ...
 async function exportJSON(current=false) {
  ...
  try {
   ...
   download('screenshots-'+data.site.id+(current?'-chapter':'')+'-v5.6.json',JSON.stringify(s,null,2))
