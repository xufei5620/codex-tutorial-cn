...
本版整合 PR #13 的基础设施与提交记录，以 PR #14 的 v5.3 课程和学习界面作为最终版本。每步骤截图 ID 保持 v5.3，不改变已有截图备份的关联规则。合并到 main 会触发现有生产 workflow，分别构建和发布 Sub2API 与 NewAPI。

验证记录应对应实际提交：运行基础设施与 v5.3 单元测试，实际构建两站并检查页面、截图管理和移动布局；分别记录源码提交、构建检查、主分支合并及线上发布状态。旧版本成功的构建不能代替本版验收。
